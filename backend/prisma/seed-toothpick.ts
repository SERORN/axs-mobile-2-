...
async function main() {
  ...
  const implantKit = await prisma.product.create({
    data: {
      ...
      requiresQuote: true, // High-value product requires quotation
    },
  });
  ...
  // Create price tiers for bulk purchasing
  await prisma.priceTier.createMany({
    data: [
      {
        variantId: variant35mm.id,
        minQty: 5,
        price: 2375.00, // 5% discount
        currency: 'MXN',
      },
      {
        variantId: variant35mm.id,
        minQty: 10,
        price: 2250.00, // 10% discount
        currency: 'MXN',
      },
      {
        variantId: variant40mm.id,
        minQty: 5,
        price: 2612.50, // 5% discount
        currency: 'MXN',
      },
      {
        variantId: variant40mm.id,
        minQty: 10,
        price: 2475.00, // 10% discount
        currency: 'MXN',
      },
    ],
  });

  // Create another product that doesn't require quotes
